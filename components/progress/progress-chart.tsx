    ...
    return (
      <div className="bg-white dark:bg-black rounded-lg px-3 py-2 shadow-xl text-sm">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-gray-500 dark:text-white text-xs font-medium">{label}</p>
...
const MemoizedBarChart = memo(({ data }: { data: DayData[] }) => (
  <ResponsiveContainer width="100%" height="100%">
    <BarChart data={data} margin={{ top: 20, right: 20, left: 20, bottom: 20 }}>
      ...
      <Bar dataKey="sessions" fill="#FFD700" radius={[4, 4, 0, 0]} maxBarSize={50} />
    </BarChart>
  </ResponsiveContainer>
))
...
export const ProgressChart = memo(() => {
  ...
  const updateData = useCallback(() => {
    try {
      ...
      setData(prevData => isDataEqual(prevData, validatedData) ? prevData : validatedData)
      setIsLoading(false)
    } catch (error) {
      console.error('Error fetching chart data:', error)
      setData([
        { label: 'Mon', sessions: 0 }, { label: 'Tue', sessions: 0 },
        { label: 'Wed', sessions: 0 }, { label: 'Thu', sessions: 0 },
        { label: 'Fri', sessions: 0 }, { label: 'Sat', sessions: 0 }, { label: 'Sun', sessions: 0 }
      ])
      setIsLoading(false)
    }
  }, [])
